...
/**
 * Lavalink AudioSourceManager for Pandora — search, tracks, albums, artists, playlists, recommendations.
 */
public class PandoraProvider extends MirroringAudioSourceManager {
    ...
    public AudioItem loadItem(AudioPlayerManager mgr, AudioReference ref) {
        ...
        try {
            if (id.startsWith(PFX_SEARCH)) { String q = id.substring(PFX_SEARCH.length()).trim(); return q.isEmpty() ? AudioReference.NO_TRACK : doSearch(q); }
            if (id.startsWith(PFX_REC)) { String t = id.substring(PFX_REC.length()).trim(); return t.isEmpty() ? AudioReference.NO_TRACK : doRecommendations(t); }
            Matcher m = URL_RE.matcher(id.trim());
            if (!m.find()) return null;
            String eid = m.group("pl") != null ? m.group("pl") : m.group("eid");
            if (eid == null) return null;
            if (eid.startsWith("TR")) return doTrack(eid);
            if (eid.startsWith("AL")) return doAlbum(eid);
    ...
    private AudioItem doSearch(String query) throws IOException {
        ...
        if (j == null) return AudioReference.NO_TRACK;
        JsonNode ann = j.get("annotations"), res = j.get("results");
        if (res == null || !res.isArray() || res.isEmpty()) return AudioReference.NO_TRACK;
        List<AudioTrack> out = new ArrayList<>();
        for (JsonNode n : res) {
            if (out.size() >= maxResults) break;
            String rid = n.asText(null); if (rid == null) continue;
    ...
    private AudioItem doTrack(String tid) throws IOException {
        JsonNode d = client.details(tid); if (d == null) return AudioReference.NO_TRACK;
    ...
    private AudioItem doAlbum(String aid) throws IOException {
        JsonNode d = client.details(aid); if (d == null) return AudioReference.NO_TRACK;
        JsonNode ann = d.get("annotations"), album = findById(aid, ann);
        if (album == null) return AudioReference.NO_TRACK;
        List<AudioTrack> tracks = new ArrayList<>();
        JsonNode tArr = album.get("tracks");
        if (tArr != null && tArr.isArray()) for (JsonNode v : tArr) {
            String id = v.asText(null); if (id == null) continue;
            JsonNode n = ann.get(id); if (n != null) { AudioTrack t = buildTrack(n, ann); if (t != null) tracks.add(t); }
        }
    ...
    private AudioItem doArtist(String artId) throws IOException {
        JsonNode d = client.details(artId); if (d == null) return AudioReference.NO_TRACK;
        JsonNode ann = d.get("annotations"), artist = findById(artId, ann);
        if (artist == null) return AudioReference.NO_TRACK;
        List<AudioTrack> tracks = new ArrayList<>();
        JsonNode top = d.at("/artistDetails/topTracks");
        if (top != null && top.isArray()) for (JsonNode v : top) {
            String id = v.asText(null); if (id == null) continue;
            JsonNode n = ann.get(id); if (n != null) { AudioTrack t = buildTrack(n, ann); if (t != null) tracks.add(t); }
        }
        ...
        return new PandoraCollection((name != null ? name : "Artist") + "'s Top Tracks", tracks,
                ExtendedAudioPlaylist.Type.ARTIST, url != null ? HOME + url : null, PandoraClient.artworkFor(artist), name, tracks.size());
    }

    private AudioItem doArtistAll(String artId) throws IOException {
        JsonNode j = client.artistAllTracks(artId); if (j == null) return AudioReference.NO_TRACK;
        JsonNode ann = j.get("annotations"), tNode = j.get("tracks");
        if (tNode == null || !tNode.isArray() || tNode.isEmpty()) return AudioReference.NO_TRACK;
        Map<String, JsonNode> pool = indexAnnotations(ann);
        List<String> ids = new ArrayList<>(); for (JsonNode v : tNode) { String s = v.asText(null); if (s != null) ids.add(s); }
        List<String> missing = new ArrayList<>(); for (String id : ids) if (!pool.containsKey(id)) missing.add(id);
        if (!missing.isEmpty()) { JsonNode extra = client.annotate(missing); for (String id : missing) { JsonNode n = extra.get(id); if (n != null) pool.put(id, n); } }
        List<AudioTrack> tracks = new ArrayList<>(); for (String id : ids) { JsonNode n = pool.get(id); if (n != null) { AudioTrack t = buildTrack(n, ann); if (t != null) tracks.add(t); } }
        JsonNode artist = findById(artId, ann);
        if (artist == null) { JsonNode d = client.details(artId); if (d != null) artist = findById(artId, d.get("annotations")); }
    ...
    private AudioItem doPlaylist(String plId) throws IOException {
        JsonNode j = client.playlistTracks(plId); if (j == null) return AudioReference.NO_TRACK;
        JsonNode ann = j.get("annotations"), tNode = j.get("tracks");
        Map<String, JsonNode> pool = indexAnnotations(ann);
        List<String> ids = new ArrayList<>();
        if (tNode != null && tNode.isArray()) for (JsonNode v : tNode) { String s = txt(v, "pandoraId"); if (s != null) ids.add(s); }
        List<String> missing = new ArrayList<>(); for (String id : ids) if (!pool.containsKey(id)) missing.add(id);
        if (!missing.isEmpty()) { JsonNode extra = client.annotate(missing); for (String id : missing) { JsonNode n = extra.get(id); if (n != null) pool.put(id, n); } }
        List<AudioTrack> tracks = new ArrayList<>(); for (String id : ids) { JsonNode n = pool.get(id); if (n != null) { AudioTrack t = buildTrack(n, ann); if (t != null) tracks.add(t); } }
        String name = txt(j, "name"), path = txt(j, "shareableUrlPath");
        String author = null; String lid = txt(j, "listenerPandoraId");
        if (lid != null && ann != null) { JsonNode a = ann.get(lid); if (a != null) author = txt(a, "fullname"); }
        return new PandoraCollection(name != null ? name : "Pandora Playlist", tracks, ExtendedAudioPlaylist.Type.PLAYLIST,
    ...
    private AudioItem doRecommendations(String tid) throws IOException {
        ...
        if (sim == null || !sim.isArray() || sim.isEmpty()) return AudioReference.NO_TRACK;
    ...
    private AudioTrack buildTrack(JsonNode n, JsonNode ann) {
        String title = txt(n, "name"); if (title == null) return null;
        String artist = txt(n, "artistName"); if (artist == null) artist = "Unknown";
        long dur = n.path("duration").asLong(0) * 1000; if (dur == 0) return null;
        ...
        if (albId != null && ann != null) { JsonNode a = ann.get(albId); if (a != null) { albName = txt(a, "name"); String p = txt(a, "shareableUrlPath"); albUrl = p != null ? HOME + p : null; } }
        String artUrl = null, artArt = null;
        String artId = txt(n, "artistId");
        if (artId != null && ann != null) { JsonNode a = ann.get(artId); if (a != null) { String p = txt(a, "shareableUrlPath"); artUrl = p != null ? HOME + p : null; artArt = PandoraClient.artworkFor(a); } }
        var info = new AudioTrackInfo(title, artist, dur, pid, false, urlPath != null ? HOME + urlPath : null, PandoraClient.artworkFor(n), isrc);
    ...
    private static JsonNode findById(String suffix, JsonNode ann) {
        ...
        var it = ann.fields();
        while (it.hasNext()) { var e = it.next(); JsonNode v = e.getValue();
            String p = txt(v, "shareableUrlPath"); if (p != null && p.endsWith("/" + suffix)) return v;
            String s = txt(v, "slugPlusPandoraId"); if (s != null && s.contains(suffix)) return v;
        }
        return null;
    }
    ...
    private static Map<String, JsonNode> indexAnnotations(JsonNode ann) {
        ...
        if (ann != null && !ann.isNull()) { var it = ann.fields(); while (it.hasNext()) { var e = it.next(); String pid = txt(e.getValue(), "pandoraId"); if (pid != null) map.put(pid, e.getValue()); } }
        return map;
    }

    private static String txt(JsonNode n, String f) {
        if (n == null || n.isNull() || !n.has(f) || n.get(f).isNull()) return null;
        String v = n.get(f).asText(null); return (v != null && !v.isEmpty()) ? v : null;
    }

    @Override
    public AudioTrack decodeTrack(AudioTrackInfo info, DataInput input) throws IOException {
        var ext = super.decodeTrack(input);
        return new PandoraTrack(info, ext.albumName, ext.albumUrl, ext.artistUrl, ext.artistArtworkUrl, ext.previewUrl, ext.isPreview, this);
    }
}
