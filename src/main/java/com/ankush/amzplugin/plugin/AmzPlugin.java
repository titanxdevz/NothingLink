...
public class AmzPlugin implements AudioPlayerManagerConfiguration, SearchManagerConfiguration {
	...
	private NothingLinkAudioEventListener audioEventListener;
	...
	public AmzPlugin(AmzPluginConfig config, InstagramPluginConfig igConfig, PandoraPluginConfig pdConfig, NothingLinkConfig nlConfig) {
		log.info("Loading NothingLink Plugin v{}...", VERSION);
		this.config = config;
		...
		if (nlConfig.isWebhookEnabled() && nlConfig.getWebhookUrl() != null && !nlConfig.getWebhookUrl().isEmpty()) {
			...
			if (nlConfig.isLogTrackEvents()) {
				this.audioEventListener = new NothingLinkAudioEventListener(webhook);
		...
		if (config.isEnabled()) {
			...
			if (webhook != null) this.amazonMusic.setWebhook(webhook);
		...
		if (igConfig.isEnabled()) {
			...
			if (webhook != null) this.igSource.setWebhook(webhook);
		...
		if (pdConfig.isEnabled()) {
			...
			if (webhook != null) this.pandoraSource.setWebhook(webhook);
		...
		}, "NothingLink-ShutdownHook"));

		log.info("NothingLink Plugin v{} loaded successfully", VERSION);
	}
	...
	@NotNull @Override public AudioPlayerManager configure(@NotNull AudioPlayerManager manager) {
		...
		if (amazonMusic != null && config.isEnabled()) {
			...
			registeredDetails.put("Amazon Music", "✅ Registered\n🔍 Search: `amzsearch:`\n📊 Limit: `" + config.getSearchLimit() + "`");
		}
	...
	@NotNull @Override public SearchManager configure(@NotNull SearchManager manager) {
		if (amazonMusic != null && config.isEnabled()) {
			...
			if (webhook != null) {
				webhook.info("🔍 Search Provider Registered", "Amazon Music search provider has been registered with LavaSearch.");
			}
		}
		return manager;
	}
	 ...
	 */
	public NothingLinkAudioEventListener getAudioEventListener() {
	...
	@PreDestroy
	public void destroy() {
		log.info("NothingLink Plugin v{} shutting down...", VERSION);
		if (webhook != null) {
			webhook.logShutdown(VERSION, "Graceful Spring Context Shutdown");
			webhook.shutdown();
		}
		log.info("NothingLink Plugin v{} shut down complete.", VERSION);
	}
}
